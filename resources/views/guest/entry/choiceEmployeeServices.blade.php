        ...
        <div class="row margin-top-light">
            @foreach($types as $key=>$type)
                <div class="row mb-2 {{$key > 0 ? 'mt-4':''}}">
                    <h3 class="my-lightgreen">{{$type->title}}</h3>
                </div>
                @foreach($employeesServices as $employeeService)
                    @if($employeeService->service->type_id === $type->id)
                        <div class="col-6 mb-3">
                            <div class="card">
                                <div class="card-body d-flex justify-content-between align-items-center">
                                    <div>
                                        <h5 class="card-title">{{$employeeService->service->title}}</h5>
                                        <p class="card-text d-flex align-items-center">
                                            {{$employeeService->service->price}} руб.
                                           ...
                                           {{$employeeService->service->time}} мин
                                        </p>
                                    </div>
                                    <a href="{{route('ChoiceDatePage', ['employee'=>$employee, 'service'=>$employeeService->service->id])}}" class="choice-button-continue">
